...
base_exp_dir=./exp/test01-${dataset}-f$fstride-t$tstride-b$batch_size-lr${lr}-${task}-${model_size}-$pretrain_exp-${pretrain_epoch}-${head_lr}x
...
do
  ...
  --tstride $tstride --fstride $fstride --warmup False --task ${task} \
  --model_size ${model_size} --adaptschedule ${adaptschedule} \
  --pretrain False --pretrain_path ${pretrain_path} \
  ...
  --lrscheduler_start 5 --lrscheduler_step 1 --lrscheduler_decay 0.85 --wa False --loss CE
done
