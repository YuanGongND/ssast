...
expdir=./exp/$expname
mkdir -p $expdir; mkdir -p ./log/emotion
...
do
  ...
  python3 run_downstream.py --expdir $expdir/unfreeze_cross-valid-on-${test_fold} -m train -u $mdl -d emotion -c config.yaml -s hidden_states -o "config.downstream_expert.datarc.test_fold='$test_fold'" -f > ./log/emotion/unfreeze_cross-valid-on-${test_fold}/${expname}_train.log
done
