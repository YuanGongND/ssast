...
class AST(nn.Module):
    def __init__(self, label_dim=527,
        ...
        if audioset_pretrain == False:
            ...
            # for mlp in [self.mlp_head, self.mlp_head, self.mlp_head]:
            #     print('initialize with weight std .02 and bias 0')
            #     trunc_normal_(mlp[1].weight, std=.02)
            #     torch.nn.init.constant_(mlp[1].bias, 0)
            ...
            if mask_patch != 0:
                print('currently in pretraining mode with {:d} masked patchs and clustering factor of {:d}'.format(
                    mask_patch, contnum))
                ...
                self.unfold = torch.nn.Unfold(kernel_size=(16, 16), stride=(16, 16))
                # print('now use two-layer prediction layer')
            ...
            if linformer == True:
                print('now use linformer blocks')
                self.v.blocks = nn.ModuleList(
                    [LinearBlock(dim=self.original_embedding_dim, num_heads=self.heads,
                                 seq_len=num_patches + self.cls_token_num) for i
                     in range(self.depth)])
            ...
            if imagenet_pretrain == True:
                ...
                new_pos_embed = self.v.pos_embed[:, self.cls_token_num:, :].detach().reshape(1,
                                                                                             self.original_num_patches,
                                                                                             self.original_embedding_dim).transpose(
                    1, 2).reshape(1, self.original_embedding_dim, self.oringal_hw, self.oringal_hw)
                ...
                if sinpos == True:
                    ...
                    new_pos_embed = nn.Parameter(
                        get_sinusoid_encoding(self.v.patch_embed.num_patches + self.cls_token_num,
                                              self.original_embedding_dim),
                        requires_grad=False)
                else:
                    print('trainable positional embedding is used.')
                    new_pos_embed = nn.Parameter(
                        torch.zeros(1, self.v.patch_embed.num_patches + self.cls_token_num,
                                    self.original_embedding_dim))
        ...
        # now load a models that is pretrained on both ImageNet and AudioSet
            ...
            if save_mdl_pos == 514:
                ...
                audio_model = AST(label_dim=527, fstride=16, tstride=16, input_fdim=128, input_tdim=1024,
                ...
                new_pos_embed = self.v.pos_embed[:, self.cls_token_num:, :].detach().reshape(1, 512,
                                                                                             self.original_embedding_dim).transpose(
                    1, 2).reshape(1, self.original_embedding_dim, 8, 64)
                ...
                if f_dim < 8:
                    new_pos_embed = new_pos_embed[:, :, 4 - int(f_dim / 2): 4 - int(f_dim / 2) + f_dim, :]
                # otherwise interpolate
                else:
                    new_pos_embed = torch.nn.functional.interpolate(new_pos_embed, size=(f_dim, t_dim), mode='bilinear')
                ...
                print('now load models from ' + pretrain_path)
                audio_model = AST(label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024,
                                          imagenet_pretrain=False, audioset_pretrain=False, model_size=model_size)
                audio_model = torch.nn.DataParallel(audio_model)
                audio_model.load_state_dict(sd, strict=False)
                self.v = audio_model.module.v
                ...
                # if the stride of the pretrained models is different with that for fine-tuning.
                if fstride != 10 or tstride != 10:
                    new_proj = torch.nn.Conv2d(1, self.original_embedding_dim, kernel_size=(fshape, tshape), stride=(fstride, tstride))
                    new_proj.weight = torch.nn.Parameter(torch.sum(self.v.patch_embed.proj.weight, dim=1).unsqueeze(1))
                    new_proj.bias = self.v.patch_embed.proj.bias
                    self.v.patch_embed.proj = new_proj

                new_pos_embed = self.v.pos_embed[:, self.cls_token_num:, :].detach().reshape(1, 1212, 768).transpose(1, 2).reshape(1, 768, 12, 101)
                # if the input sequence length is larger than the original audioset (10s), then cut the positional embedding
                if t_dim < 101:
    ...
    def create_mask(self, edge=1, device='cpu'):
        mask = torch.ones([1024, 128], device=device)
        for e in range(0, edge):
            mask[15 - e::16, :] = 0
            mask[15 + e + 1::16, :] = 0
        for e in range(0, edge):
            mask[:, 15 - e::16] = 0
    ...
    def mpc(self, x):
        ...
        encode_samples = torch.empty((B, self.mask_patch, self.original_embedding_dim), device=x.device,
        ...
        for i in range(B):
            ...
            # mask_index[i] = self.gen_maskid(512, self.mask_patch, self.contnum)
        ...
        pred = torch.empty((B, self.mask_patch, self.original_embedding_dim),
                           device=x.device).float()  # e.g. size 12*100*768
        for i in range(B):
            #  +2 for indexws because cls and dis token
        ...
        for i in np.arange(0, B):
            ...
            correct = torch.sum(
                torch.eq(torch.argmax(self.softmax(total), dim=0), self.mask_correct))  # correct is a tensor
            nce += torch.sum(torch.diag(self.lsoftmax(total)))  # nce is a tensor
        acc = 1. * correct / self.mask_patch
        nce /= -1. * B * self.mask_patch

        return acc, nce

    # mask patch embedding is learnable
    def mpclearnpatch(self, x, show_mask=False):
        ...
        # encode the patch
        # size 12(batch_size) * 100(#mask_patch) * 768(hidden_dim), save the true embedding of masked samples
        encode_samples = torch.empty((B, self.mask_patch, 256), device=x.device, requires_grad=False).float()
        # size 12(batch_size) * 100(#mask_patch), index of masked patches
        mask_index = torch.empty((B, self.mask_patch), device=x.device, requires_grad=False).long()
        # size 12(batch_size) * 576(sequence_len) * 768(hidden_dim)
        mask_dense = torch.ones([x.shape[0], x.shape[1], x.shape[2]], device=x.device)
        ...
        for i in range(B):
            ...
            # mask_index[i] = self.gen_maskid(512, self.mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_square(512, self.mask_patch, self.contnum)
            mask_index[i] = self.gen_maskid_square4(512, self.mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_energy(512, self.mask_patch, self.contnum, input[i])
            # copy the masked embeddings, note gradients are stopped in this path
        ...
        # directly use the AST output of the masked positions as the prediction of the masked embedding
        pred = torch.empty((B, self.mask_patch, 256), device=x.device).float()  # e.g. size 12*100*768
        for i in range(B):
            #  +2 for indexws because cls and dis token
            pred[i] = self.cpredlayer(x[i, mask_index[i] + 2, :])
        ...
        for i in np.arange(0, B):
            ...
            correct += torch.sum(
                torch.eq(torch.argmax(self.softmax(total), dim=0), self.mask_correct))  # correct is a tensor
            nce += torch.sum(torch.diag(self.lsoftmax(total)))  # nce is a tensor
        acc = 1. * correct / (B * self.mask_patch)
        nce = nce / (-1. * B * self.mask_patch)
        ...
        if show_mask == False:
            ...
            for i in range(B):
                ...
                pred[i, mask_index[i], :] = torch.tensor(result).reshape(self.mask_patch, 1).expand(self.mask_patch,
                                                                                                    256)
            ...
            fold = torch.nn.Fold(output_size=([128, 1024]), kernel_size=(16, 16), stride=(16, 16))
    ...
    def mpclearnpatchrand(self, x, show_mask=False, eval_mask=-1):
        ...
        for i in range(B):
            ...
            # mask_index[i] = self.gen_maskid(512, self.mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_square(512, self.mask_patch, self.contnum)
            mask_index[i] = self.gen_maskid_square4(512, cur_mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_energy(512, self.mask_patch, self.contnum, input[i])
            # copy the masked embeddings, note gradients are stopped in this path
            encode_samples[i] = input[i, mask_index[i], :].clone().detach()
            # mask the encode samples with 0
        ...
        for i in np.arange(0, B):
            ...
            correct += torch.sum(torch.eq(torch.argmax(self.softmax(total), dim=0),
                                          torch.arange(0, cur_mask_patch, device=x.device)))  # correct is a tensor
            nce += torch.sum(torch.diag(self.lsoftmax(total)))  # nce is a tensor
    ...
    # mask patch embedding is learnable
    def mpclearnembed(self, x, show_mask=False):
        input = self.unfold(x).transpose(1, 2)
        B = x.shape[0]
        # x in shape (batch_size, sequence_len, embedding dim)
        x = self.v.patch_embed(x)
        ...
        for i in range(B):
            ...
            # mask_index[i] = self.gen_maskid(512, self.mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_square(512, self.mask_patch, self.contnum)
            mask_index[i] = self.gen_maskid_square3(512, self.mask_patch, self.contnum)
            # mask_index[i] = self.gen_maskid_energy(512, self.mask_patch, self.contnum, input[i])
            # copy the masked embeddings, note gradients are stopped in this path
            encode_samples[i] = x[i, mask_index[i], :].clone().detach()
            # mask the encode samples with 0
    ...
    # masked patch prediction
    # if show_mask = True, return the actual mask and correct predictions for mpc.
    def mpcpatch(self, x, show_mask=False):
        input = self.unfold(x).transpose(1, 2)
        B = x.shape[0]
        # x in shape (batch_size, sequence_len, embedding dim)
        x = self.v.patch_embed(x)
        ...
        if show_mask == False:
            ...
            for i in range(B):
                pred[i, mask_index[i], :] = torch.eq(torch.argmax(self.softmax(total), dim=0), self.mask_correct)[i]
    ...
    def mpg(self, input):
        ...
        for i in range(B):
            # randomly generate #mask_patch mask indexes without duplicate
            # mask_index[i] = torch.randperm(self.num_patches)[:self.mask_patch]
            # mask_index[i] = self.gen_maskid_square2(512, self.mask_patch, self.contnum)
            mask_index[i] = self.gen_maskid_square4(512, self.mask_patch, self.contnum)
            # mask the encode samples with 0
            mask_dense[i, mask_index[i], :] = 0
        ...
        # directly use the AST output of the masked positions as the prediction of the masked embedding
        pred = torch.empty((B, self.mask_patch, self.fshape * self.tshape),
                           device=x.device).float()  # e.g. size 12*100*256
        target = torch.empty((B, self.mask_patch, self.fshape * self.tshape),
                             device=x.device).float()  # e.g. size 12*100*256
    ...
    def mpgrand(self, input, eval_mask=-1):
        ...
        # directly use the AST output of the masked positions as the prediction of the masked embedding
        pred = torch.empty((B, cur_mask_patch, self.fshape * self.tshape),
                           device=x.device).float()  # e.g. size 12*100*256
        target = torch.empty((B, cur_mask_patch, self.fshape * self.tshape),
                             device=x.device).float()  # e.g. size 12*100*256
    ...
    def getrep(self, x):
        ...
        # output sharp in [batch_size, patch_num, embed_dim]
        return x
