...
class ASTModelUni(nn.Module):
    def __init__(self, label_dim=527,
        ...
        super(ASTModelUni, self).__init__()
        ...
        if audioset_pretrain == False:
            ...
            if linformer == True:
                ...
                self.v.blocks = nn.ModuleList(
                    [LinearBlock(dim=self.original_embedding_dim, num_heads=self.heads, seq_len=num_patches + self.cls_token_num) for i
                     in range(self.depth)])
        ...
        # now load a models that is pretrained on both ImageNet and AudioSet
            ...
            if save_mdl_pos == 514:
                ...
                audio_model = ASTModelUni(label_dim=527, fstride=16, tstride=16, input_fdim=128, input_tdim=1024, imagenet_pretrain=False, audioset_pretrain=False, model_size=model_size)
                ...
                new_pos_embed = self.v.pos_embed[:, self.cls_token_num:, :].detach().reshape(1, 512, self.original_embedding_dim).transpose(1, 2).reshape(1, self.original_embedding_dim, 8, 64)
                ...
                if f_dim < 8:
                    new_pos_embed = new_pos_embed[:, :, 4 - int(f_dim/2): 4 - int(f_dim/2) + t_dim, :]
                # otherwise interpolate
                else:
                    new_pos_embed = torch.nn.functional.interpolate(new_pos_embed, size=(f_dim, t_dim), mode='bilinear')
    ...
    def gen_maskid_square4(self, sequence_len=512, mask_size=100, cont=3):
        ...
        while len(list(set(mask_id))) <= mask_size:
            start_id = randrange(sequence_len)
            # added in the revision, to save some time
    ...
    def mpc(self, x):
        ...
        encode_samples = torch.empty((B, self.mask_patch, self.original_embedding_dim), device=x.device, requires_grad=False).float()
    ...
    def mpclearnembed(self, x, show_mask=False):
        ...
        # directly use the AST output of the masked positions as the prediction of the masked embedding
        pred = torch.empty((B, self.mask_patch, self.original_embedding_dim), device=x.device).float()  # e.g. size 12*100*768
        for i in range(B):
            #  +2 for indexws because cls and dis token
            pred[i] = self.cpredlayer(x[i, mask_index[i] + 2, :])
    ...
    def mpg(self, input):
        ...
        # directly use the AST output of the masked positions as the prediction of the masked embedding
        pred = torch.empty((B, self.mask_patch, self.fshape * self.tshape), device=x.device).float()  # e.g. size 12*100*256
        target = torch.empty((B, self.mask_patch, self.fshape * self.tshape), device=x.device).float() # e.g. size 12*100*256
...
if __name__ == '__main__':
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(device)
    ast_mdl_u = ASTModelUni(imagenet_pretrain=False, mask_patch=200, contnum=5, model_size='tiny224')
    ast_mdl_p = ASTModelUni(imagenet_pretrain=True, mask_patch=200, contnum=5, model_size='tiny224')
    #ast_mdl = torch.nn.DataParallel(ast_mdl)
    # ast_mdl = ast_mdl.to(device)
    #test_input = torch.zeros([1, input_tdim, 128]).to(device)
    #acc, nce = ast_mdl(test_input, task='mpc')
