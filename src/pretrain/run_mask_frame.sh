...
batch_size=24
lr_patience=2
exp_dir=./exp/mask35r-${set}-${model_size}-p$imagenetpretrain-b$batch_size-lr${lr}-m${mask_patch}-contmask${contnum}-${task}-framenocluster-asli

#mkdir -p $exp_dir

CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run_frame.py --model ${model} --dataset ${dataset} \
